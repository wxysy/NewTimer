...
namespace TimerLib
{
    public class TimerStarter
    {
        ...
        /// <param name="beforeTimerWindowClosedEvent">倒计时器窗体在关闭之前的操作(e:剩余秒数，若0时刻关闭也会引发此事件)</param>
        /// <param name="timerTickEvent">计时器每次Tick时的额外操作(不用在此类中编写倒计时变化，0时刻会引发专门的0时刻事件，0时刻不会引发此事件)</param>
        /// <returns>倒计时器实例</returns>
        public static CountDownTimer CreatCountDownTimer(int countDownSeconds, Brush countDownColor, int warningSeconds, Brush warningColor, int timerInterval, bool allowUIOperation, EventHandler<int>? timerTickEvent, EventHandler? zeroEvent, EventHandler<int>? beforeTimerWindowClosedEvent)
        {
            ...
            countDown.CDT_BeforeTimerWindowClosedEvent += beforeTimerWindowClosedEvent;
            return countDown;
        }
    }
}
